...
void Nonbonded::analysis_cells(const Initial *init, const Vector *pos, Vector *f, const Configure *conf, const double* aBox) {
    ...
    */
    ...
    ncellx = int(aBox[0]/pairdist) + 1;
    ncelly = int(aBox[1]/pairdist) + 1;
    ncellz = int(aBox[2]/pairdist) + 1;
...
void Nonbonded::analysis_nbrlist(const Initial *init, const Vector *pos, Vector *f, const double* aBox){
    
    double box_2[3];
    box_2[0] = aBox[0]*0.5; box_2[1] = aBox[1]*0.5; box_2[2] = aBox[2]*0.5;
    ...
    for (int i=0; i<natoms; i++){ // For a box with the origin at the lower left vertex
        poshift[i].x = pos[i].x - floor(pos[i].x/aBox[0]) * aBox[0];
        poshift[i].y = pos[i].y - floor(pos[i].y/aBox[1]) * aBox[1];
        poshift[i].z = pos[i].z - floor(pos[i].z/aBox[2]) * aBox[2];
    }
    ...
    double pairdistx = aBox[0]/ncellx + 0.001; 
    double pairdisty = aBox[1]/ncelly + 0.001; 
    double pairdistz = aBox[2]/ncellz + 0.001;
...
void Nonbonded::analysis_threebody_nbrlist(const Initial *init, const Vector *pos, Vector *f, const double* aBox){
    /**
...
void Nonbonded::analysis_comp(const Initial *init, const Vector *pos,
...
void Nonbonded::analysis_comp_threebody(const Initial *init, const Vector *pos,
            ...
            if (distij < threebody_ijcut2) {
                for (int k = j+1; k < atoms[iatom].nbrlist.size(); k++) {
                    ...
                    if (iatom_res != katom_res) {
                        ...
                        // PBC
                        apply_pbc(aBox, box_2, dik);
                        apply_pbc(aBox, box_2, djk);
